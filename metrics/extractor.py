...
class VitExtractor:
    ...
    def get_queries_from_qkv(self, qkv, input_img_shape):
        ...
        q = qkv.reshape(batch_size,patch_num, 3, head_num, embedding_dim // head_num).permute(0, 2, 3, 1, 4)[:,0]
        return q
    ...
    def get_keys_from_qkv(self, qkv, input_img_shape):
        ...
        head_num = self.get_head_num()
        embedding_dim = self.get_embedding_dim()
        k = qkv.reshape(batch_size, patch_num, 3, head_num, embedding_dim // head_num).permute(0, 2, 3, 1, 4)[:,1]
        return k
