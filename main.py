...
def main(model, model_number, grad):
    ...
    for img_dir in tqdm(image_dirs):
        ...
        for k,v in conditions.items():
            ...
            from_clip_embedding = from_text_emb.repeat(batch_size,1)

            model_input = torch.cat([img_embs,
                                    from_clip_embedding,
                                    to_clip_embedding], dim=1).view(batch_size, 3, -1)
...
@torch.no_grad()
def blip_experiments_main(grad):
    ...
    conditions = [' on a summer day',
                  ...
                  ' at daytime']
    conditions = ['a photograph of a street'+ con for con in conditions]
    save_root = Path('check_valid/blip_experiments')
    save_root.mkdir(exist_ok=True)
    
    save_valid = save_root/f'202503111041-{grad}'
    save_valid.mkdir(exist_ok=True)
    ...
    for img_dir in tqdm(image_dirs):
        ...
        for i,t in enumerate(conditions):
            ...
            for g in range(2):
                guidance_tensor = torch.arange(g*30+2.5, (g+1)*30+2.5, 2.5).view(-1,1).to('cuda')
                batch_size = len(guidance_tensor)
                blip_prompt = generate_prompt([image])*batch_size
                pnp_multi_cons = [blip_prompt,[t]*batch_size]
                pred_ginit = guidance_tensor+5.0
                portion1 = 5.0/pred_ginit
                portion2 = 1-portion1
                pred_portion = torch.cat([portion1, portion2], dim=-1)
                gs = pred_ginit*pred_portion
                time_g = gs[:,0]
                weather_g = gs[:,1]
                ...
                for i in range(batch_size):
                    g = str(guidance_tensor[i][0].item()).replace('.','_')
                    t_g = f'{t_values.item(i):.1f}'.replace('.','_')
                    w_g = f'{w_values.item(i):.1f}'.replace('.','_')
                    save_gen_img = save_time/f'guid-{t_g}-{w_g}-prompt-{blip_prompts[i]}-{weather_prompts[i]}.png'
                    gen_images[i].save(save_gen_img)  
...
if __name__ == '__main__':   
    ...
    blip_experiments_main('decrease')'''
    
if __name__ == '__main__':
    model_path = Path('ckpts/0306174136_Attention_model.pt')
