...
def main(args):
    ...
    ip2p_augmented_prompts = get_json(args.ip2p_augmented_prompts)
    
    candidates = ip2p_augmented_prompts[target_domain]
    ...
    devide_guide = base_setting['devide_guide']
    ...
    outputs = pipe(prompt=candidates,
                   image=images,
                   num_inference_steps = n_timestep,
                   guidance_scale=guidance,
                   image_guidance_scale=2.0,
                   devide_guide = devide_guide)
    ...
    save_image = save_dir/f'ip2p_{prompt}.png'
    max_image.save(save_image) 
    
    candidate_dir = save_dir/'candidates87'
    candidate_dir.mkdir(exist_ok=True, parents=True)
    for i, img in enumerate(gen_images):
        img.save(candidate_dir/f'ip2p_{i}_{candidates[i]}.png')
